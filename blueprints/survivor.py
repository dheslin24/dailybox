...
def sv_create_pool():
    ...
    if request.method == 'POST':
        ...
        if not pool_name or not pool_password:
            ...
            exists = db2("SELECT pool_id FROM sv_pools WHERE pool_name = %s", (pool_name,))
            if exists:
                error = f'A pool with the name "{pool_name}" already exists. Please choose a different name.'
                pool_id = None
            else:
                try:
                    db2("INSERT INTO sv_pools (pool_name, password, admin) VALUES (%s, %s, %s)", (pool_name, pool_password, user_id))
                    result = db2("SELECT pool_id FROM sv_pools WHERE pool_name = %s", (pool_name,))
                    pool_id = result[0][0] if result else None
...
def survivor_pool():
    ...
    if user_id:
        ...
        if pool_ids:
            ...
            user_pools = db2(f"SELECT pool_id, pool_name FROM sv_pools WHERE pool_id IN ({pool_ids_str})")
...
def submit_team():
    ...
    logging.info("Team submitted: %s Week: %s pool ID %s Season: %s", team, week, pool_id, season)
...
def team_click():
    ...
    logo = request.form.get('logo')
    week = request.args.get('week', default=1, type=int)
    logging.info("Team clicked: %s Game ID: %s", team, game_id)
...
def survivor_pool_picks():
    ...
    for row in pick_rows:
        ...
        for game in games_by_week[week]:
            if team == game.get('home_team') or team == game.get('away_team'):
                logging.info("Found game for team %s in week %s", team, week)
                if 'start_date' in game and game['start_date']:
                    dt_str = game['start_date'].replace('Z', '')
                    try:
                        dt_utc = datetime.strptime(dt_str, '%Y-%m-%dT%H:%M')
                ...
                if game.get('winner_team'):
                    logging.info("Game winner: %s", game['winner_team'])
...
def join_pool():
    ...
    if found:
        ...
        exists = db2("SELECT 1 FROM sv_user_pools WHERE user_id = %s AND pool_id = %s", (user_id, pool_id_val))
        if not exists:
            db2("INSERT INTO sv_user_pools (user_id, pool_id, active) VALUES (%s, %s, 1)", (user_id, pool_id_val))
            logging.info("User %s added to pool %s", user_id, pool_id_val)
        else:
            logging.info("User %s is already in pool %s", user_id, pool_id_val)
        return redirect(url_for('survivor.survivor_teams_selected', pool_id=pool_id_val))
    else:
        logging.warning("No matching pool for ID=%s, Name=%s", pool_id, pool_name)
